...
func TestJSONRPC_RejectsMalformedDuplicateNotificationAndDisallowedBatches(t *testing.T) {
	...
	cases := []struct{ name, body, reason string }{
		...
		{"write", `{"jsonrpc":"2.0","id":1,"method":"eth_sendRawTransaction","params":["0x00"]}`, "method"},
...
func TestJSONRPC_RejectsTransportAndEndpointBypasses(t *testing.T) {
	...
	cases := []struct {
		...
		{"incomplete", func(r *http.Request) { r.ContentLength++ }, "body_oversize"},
		{"oversize", func(r *http.Request) {}, "body_oversize"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestJSONRPC_RejectsIncompleteBodyWithinLimit(t *testing.T) {
	p := testPolicy(t, 1024)
	req := request(t, "POST", "https://rpc.example/rpc", `{"jsonrpc":"2.0","id":1,"method":"eth_chainId"}`)
	req.ContentLength++
	action, annotations := decision(t, p, req)
	require.Equal(t, transform.ActionReject, action)
	require.Equal(t, "body_incomplete", annotations["reason"])
}

func TestJSONRPC_CONNECTIsTransportOnlyAndResponseIsNoop(t *testing.T) {
	p := testPolicy(t, 0)
	...
	result, err := p.TransformResponse(context.Background(), &transform.TransformContext{}, req, response)
	require.NoError(t, err)
